...
Ext.extend(Ext.ux.grid.GridDropZone, Ext.dd.DropZone, {
    ...
    onNodeOver : function(target, dd, e, data){
        ...
        if(this.isValidDropPoint(target, pt, dd, e, data)){
           if(pt == "above"){
               returnCls = this.view.findItemByChild(target) === 0 ? "x-tree-drop-ok-above" : "x-tree-drop-ok-between";
               cls = "x-grid3-drag-insert-above";
           }else if(pt == "below"){
               returnCls = this.view.findItemByChild(target) === this.grid.getStore().getCount() -1 ? "x-tree-drop-ok-below" : "x-tree-drop-ok-between";
