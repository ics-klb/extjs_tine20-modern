...
Klb.system.widgets.customfields.ConfigManager = function() {
    var stores = {};
    
    var getStore = function(app) {
        app = Klb.system.appMgr.get(app);
        if (! stores[app.appName]) {
            var allCfs = (Ext.isFunction(app.getRegistry)) ? app.getRegistry().get('customfields') : null;
            stores[app.appName] = new Klb.system.data.JsonStore({
                model: 'Klb.system.Model.Customfield',
                data: allCfs ? allCfs : []
            });
