...
Klb.system.widgets.customfields.EditDialogPlugin = function (config) {
    Ext.apply(this, config);
};

Klb.system.widgets.customfields.EditDialogPlugin.prototype = {
    /**
    ...
    onRecordLoad: function() {
        ...
        for (name in this.customfieldsValue) {
            ...
            cfConfig = Klb.system.widgets.customfields.ConfigManager.getConfig(this.app, modelName, name);
            
            if (cfConfig) {
                // transform datetime values
                if (['date', 'datetime'].indexOf(Ext.util.Format.lowercase(cfConfig.get('definition').type)) != -1) {
                    this.customfieldsValue[name] = Ext.Date.parse(this.customfieldsValue[name], Date.patterns.ISO8601Long);
                }
                
                if (field) {
                    if(field.isXType('combo') && Ext.isObject(this.customfieldsValue[name])) {
                        var phpClassName = cfConfig.get('model').split('_'),
                            recordClass = Klb.App[phpClassName[0]].Model[phpClassName[2]],
    ...
    onBeforeRender: function() {
        var modelName = this.editDialog.recordClass.getMeta('appName') + '_Model_' + this.editDialog.recordClass.getMeta('modelName'),
            allCfConfigs = Klb.system.widgets.customfields.ConfigManager.getConfigs(this.app, modelName),
    ...
    addCFTab: function(cfConfigs) {
        ...
        cfConfigs.each(function(cfConfig) {
            try {
                ...
                fieldObj = Klb.system.widgets.customfields.Field.get(this.app, cfConfig, {anchor: '95%'}, this.editDialog);
                uiConfig = definition.uiconfig ? definition.uiconfig : {};
                group = uiConfig.group ? uiConfig.group : _('General');
                
                Klb.Logger.debug('Klb.system.widgets.customfields.EditDialogPlugin::addCFTab() - Adding ' + cfConfig.get('name') + ' to group ' + group);
                this.getFieldSet(group).add(fieldObj);
                
            } catch (e) {
                Klb.Logger.debug(e);
                Klb.Logger.err('Unable to create custom field "' + cfConfig.get('name') + '". Check definition!');
            }
        }, this);
        
        this.cfTab  = new Ext.panel.Panel({
...
Ext.preg('tinebase.widgets.customfield.editdialogplugin', Klb.system.widgets.customfields.EditDialogPlugin);
