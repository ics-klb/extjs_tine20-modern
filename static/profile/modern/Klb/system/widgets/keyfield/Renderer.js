 ...
 * @class       Klb.system.widgets.keyfield.Renderer
 * @singleton
 */
Klb.system.widgets.keyfield.Renderer = function(){
    var renderers = {};
    ...
    return {
        ...
        get: function(app, keyFieldName, what) {
            var appName = Ext.isString(app) ? app : app.appName,
                ...
                store = Klb.system.widgets.keyfield.StoreMgr.get(app, keyFieldName),
            ...
            if (! renderers[key]) {
                renderers[key] = function(id) {
                    ...
                    if (whatParts.indexOf('icon') > -1 && icon) {
                        string = string + '<img src="' + Klb.Common.RESOURCE_PATH + icon + '" class="apibase-keyfield-icon" ext:qtip="' + Ext.util.Format.htmlEncode(i18nValue) + '" />';
                    }
