 ...
 * @class       Klb.system.widgets.dialog.TokenModeEditDialogPlugin
 * @author      Cornelius Weiss <c.weiss@metaways.de>
 * @constructor
 * @param {Object} config The configuration options.
 */
Klb.system.widgets.dialog.TokenModeEditDialogPlugin = function(config) {
    Ext.apply(this, config);
};

Klb.system.widgets.dialog.TokenModeEditDialogPlugin.prototype = {
    ...
    init : function(editDialog) {
        ...
        this.app = Klb.system.appMgr.get(this.editDialog.app);
        ...
        Ext.Function.createSequence(this.editDialog.onRecordLoad, this.onRecordLoad, this);
    },
    ...
    startTokenMode: function() {
        ...
        this.form.items.each(function(item) {
            if (item instanceof Ext.form.field.Text && ! item.disabled && ! item.forceSelection) {
                if (item.rendered) {
    ...
    endTokenMode: function() {
        if (this.inTokenMode) {
            ...
            Ext.select('div[class^=apibase-tokenedit-tokenbox]', this.editDialog.getEl()).remove();
    ...
    initialize: function() {
        if (! this.isInitialized) {
            ...
            this.dragZone = new Ext.dd.DragZone(this.editDialog.getEl(), {
                getDragData: this.getDragData.bind(this),
            ...
            this.dropZone = new Ext.dd.DropZone(this.editDialog.getEl(), {
                ...
                getTargetFromEvent: this.getTargetFromEvent.bind(this)
            });
    ...
    tokenizeField: function(field) {
        ...
        var el = field.itemCt.insertFirst({
            'tag': 'div',
            'class': 'apibase-tokenedit-tokenbox x-form-text' + (field instanceof Ext.form.TextArea ? ' apibase-tokenedit-tokenbox-area' : '')
        });
    ...
    syncFieldSize: function(field) {
        var el = field.itemCt.down('div[class^=apibase-tokenedit-tokenbox]');
    ...
    getTokenTpl: function() {
        if (! this.tokenTpl) {
            this.tokenTpl = new Ext.XTemplate(
                '<tpl for=".">',
                    '<span class="apibase-tokenedit-token">{.}</span>',
                '</tpl>'
            ).compile();
        }
    ...
    onClick: function(e) {
        ...
        var target = e.getTarget('.apibase-tokenedit-token', 10, true);
        if (target) {
            if (e.ctrlKey) {
                if (this.selection.indexOf(target) < 0) {
                    this.selection.push(target);
                    target.addClass('apibase-tokenedit-token-selected');
                } else {
                    this.selection.remove(target);
                    target.removeClass('apibase-tokenedit-token-selected');
                }
            } else {
                Ext.each(this.selection, function(el) {
                    el.removeClass('apibase-tokenedit-token-selected');
                }, this);
    ...
    getDragData: function(e) {
        ...
        if (target) {
            ...
            Ext.each(this.selection, function(el) {
                sourceFields.push(Ext.getCmp(el.parent('div[class^=apibase-tokenedit-tokenbox]').parent('div').child('*[id^=ext-comp]').id));
    ...
    getTargetFromEvent: function(e) {
        var target = e.getTarget('.apibase-tokenedit-tokenbox', 10, true);
    ...
    onNodeOver: function(target, dd, e, data) {
        var tokens = target.query('.apibase-tokenedit-token'),
            nextToken = null,
            nextTokenBox = null,
            eventXY = e.getXY();
    ...
    onNodeDrop : function(target, dd, e, data) {
        ...
        Ext.each([field].concat(data.sourceFields), function(f) {
            ...
            Ext.each(f.itemCt.query('.apibase-tokenedit-token'), function(el) {
    ...
    getPositionEl: function() {
        if (! this.positionEl) {
            this.positionEl = this.editDialog.getEl().createChild({
                'tag':  'span',
                'class': 'apibase-tokenedit-position',
                'html' : '&nbsp;'
            });
        }
