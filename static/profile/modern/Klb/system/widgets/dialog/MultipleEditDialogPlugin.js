 ...
 * @class       Klb.system.widgets.dialog.MultipleEditDialogPlugin
 * @author      Alexander Stintzing <a.stintzing@metaways.de>
 * 
 * @plugin for Klb.system.widgets.editDialog
 */
Klb.system.widgets.dialog.MultipleEditDialogPlugin = function(config) {
    Ext.apply(this, config);
};

Klb.system.widgets.dialog.MultipleEditDialogPlugin.prototype = {
    /**
    ...
    init : function(ed) {
        ...
        this.app = Klb.system.appMgr.get(ed.app);
     ...
     * call this from the component to disable by: Klb.system.widgets.dialog.MultipleEditDialogPlugin.prototype.registerSkipItem(this);
     * @param {} item
     */
    ...
    onAfterRender : function() {
        ...
        Ext.each(this.editDialog.recordClass.getFieldNames(), function(key) {
            ...
            if (!field) {
                Klb.Logger.info('No field found for property "' + key + '". Ignoring...');
                return true;
            }
            Klb.Logger.info('Field found for property "' + key + '".');
            keys.push({key: key, type: 'default', formField: field, recordKey: key});
        }, this);
        
        // get customfields to handle
        var cfConfigs = Klb.system.widgets.customfields.ConfigManager.getConfigs(this.app, this.editDialog.recordClass);
        if (cfConfigs) {
            Ext.each(cfConfigs, function (config) {
                var field = this.form.findField('customfield_' + config.data.name);
                if (!field) {
                    Klb.Logger.info('No customfield found for property "' + config.data.name + '". Ignoring...');
                    return true;
                }
                Klb.Logger.info('Customfield found for property "' + config.data.name + '".');
        ...
        if (this.editDialog.relationPickers) {
            Ext.each(this.editDialog.relationPickers, function(picker) {
                Klb.Logger.info('RelationPicker found. Using "' + '%' + picker.relationType + '-' + picker.fullModelName + '" as key.');
        ...
        Ext.each(keys, function(field) {
            ...
            if ((!(ff.isXType('textfield'))) && (!(ff.isXType('checkbox'))) && (!(ff.isXType('datetimefield'))) || ff.multiEditable === false) {
                Klb.Logger.debug('Disabling field for key "' + field.recordKey + '". Cannot be handled atm.');
                ff.setDisabled(true);
            ...
            } else if (ff.isXType('apirecordpickercombobox')) {
    ...
    onInitField: function(ff) {
        Klb.Logger.info('Init handler called for field "' + ff.name + '".');
        ...
        if (! ff.multipleInitialized) {
            ...
            Klb.Logger.info('Setting start value to "' + startValue + '".');
    ...
    createMultiButton: function(formField) {
        var subLeft = 18;
        if (formField.isXType('apirecordpickercombobox')) {
        ...
        } else if (formField.isXType('Klb.widget.field.AutoCompleteField')) {
        ...
        var el = formField.el.parent().select('.apibase-editmultipledialog-clearer'), 
        ...
        formField.multiButton.set({
            ...
            'class': 'apibase-editmultipledialog-clearer',
            'style': 'left:' + left
            });
    ...
    onMultiButton: function() {
        Klb.Logger.debug('Multibutton called.');
        // scope: formField
        if (this.multiButton.hasClass('undo')) {
            Klb.Logger.debug('Resetting value to "' + this.startingValue + '".');
            if (this.startRecord) {
            ...
            if (this.isXType('extuxclearabledatefield') && this.multi) {
                ...
                var parent = this.el.parent().select('.apibase-editmultipledialog-clearer');
            ...
            Klb.Logger.debug('Clearing value.');
            if (this.isXType('extuxclearabledatefield') && this.multi) {
                var startLeft = this.startLeft ? this.startLeft : 0;
                startLeft -= 17;
                var parent = this.el.parent().select('.apibase-editmultipledialog-clearer');
    ...
    onTriggerField: function() {
        // scope on formField
        Klb.Logger.info('Trigger handler called for field "' + this.name + '".');
        var ar = this.el.parent().select('.apibase-editmultipledialog-dirty');
        ...
        Klb.Logger.info('Start value: "' + originalValue + '", current: "' + currentValue + '"');
        if ((Ext.encode(originalValue) != Ext.encode(currentValue)) || (this.cleared === true)) {  // if edited or cleared
            ...
            if (ar.elements.length > 0) {
                ...
                arrow.set({
                    'src': Ext.BLANK_IMAGE_URL,
                    'class': 'apibase-editmultipledialog-dirty',
                    'height': 5,
                    'width': 5
                });
            ...
            // Set field
            this.edited = true;
            this.removeClass('apibase-editmultipledialog-noneedit');
            ...
            this.edited = false;
            if (this.multi) {
                this.addClass('apibase-editmultipledialog-noneedit');
            }
...
//        }
        ...
        this.editDialog.window.setTitle( Ext.String.format(_('Edit {0} {1}'), this.totalRecordCount, this.editDialog.i18nRecordsName));

        Klb.Logger.debug('loading of the following intermediate record completed:');
        Klb.Logger.debug(this.interRecord);
    ...
    setFieldValue: function(field, samevalue) {
        ...
        if (! samevalue) {  // The records does not have the same value on this field
            ff.addClass('apibase-editmultipledialog-noneedit');
    ...
    wrapCheckbox: function(checkbox) {
        ...
        checkbox.getEl().wrap({tag: 'span', 'class': 'apibase-editmultipledialog-dirtycheck'});
    ...
    onRecordUpdate: function() {
        ...
        Ext.each(this.changedFields, function(field) {
            ...
            if (ff.isXType('checkbox')) {
                    ...
                    renderer = Klb.system.Common.minutesRenderer;
                }
        ...
        if (changes.length > 0) {
            Ext.MessageBox.confirm(_('Confirm'),  Ext.String.format(_('Do you really want to change these {0} records?') + this.changedHuman, this.totalRecordCount),
                function(_btn) {
                if (_btn == 'yes') {
                    ...
                    Klb.system.Ajax.request({
                        ...
                        params: {
                            method: 'Api.updateMultipleRecords',
                        ...
                        success: function(_result, _request) {
                            ...
                            if (resp.failcount > 0) {
                                var window = Klb.system.widgets.dialog.MultipleEditResultSummary.openWindow({
...
//Ext.ComponentManager.create('multiple_edit_dialog', Klb.system.widgets.dialog.MultipleEditDialogPlugin);
