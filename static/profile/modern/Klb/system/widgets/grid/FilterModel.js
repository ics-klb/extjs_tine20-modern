 ...
 * @class       Klb.system.widgets.grid.FilterModel
 * @extends     Ext.util.Observable
 * @constructor
 */
Ext.define('Klb.system.widgets.grid.FilterModel', {
    ...
    operatorRenderer: function (filter, el) {
        var operatorStore = new Klb.system.data.JsonStore({
        ...
        if (this.operators.length > 0) {
            operatorStore.each(function(operator) {
                if (operator && this.operators.indexOf(operator.get('operator')) < 0 ) {
        ...
        if (operatorStore.getCount() > 1) {
            var operator = new Ext.form.field.ComboBox({
                ...
                tpl:  '<ul class="x-list-plain">'
                        + '<tpl for="."><li role="option" class="x-boundlist-item x-combo-list-item tw-ftb-operator-{operator}">{label}</li></tpl>'
                    + '</ul>',
                renderTo: el
            });
    ...
    valueRenderer: function(filter, el) {
        ...
        switch (this.valueType) {
            ...
            case 'percentage':
                ...
                break;
            // case 'user':
            //     value = new Klb.Addressbook.SearchCombo(Ext.apply(commonOptions, {
            //         listWidth: 350,
            ...
            case 'country':
                ...
                value = new Klb.system.widgets.CountryCombo(Ext.apply(commonOptions, {
                }));
                break;
            case 'customfield':
            case 'string':
    ...
    onFiltertrigger: function() {
        ...
        if (parseInt(Klb.App.Api.registry.get('preferences').get('filterChangeAutoSearch'), 10) === 1) {
 ...
 * @singleton
 */
Klb.system.widgets.grid.FilterRegistry = function() {
    var filters = {};
