 ...
 * @class       Klb.system.widgets.grid.ForeignRecordFilter
 ...
 * @constructor
 */
Ext.define('Klb.system.widgets.grid.ForeignRecordFilter', {
    ...
    initComponent: function() {
        if (this.foreignRecordClass) {
            this.foreignRecordClass = Klb.system.data.RecordMgr.get(this.foreignRecordClass);
        }
    ...
    initGeneric: function() {
        ...
        if (this.ownRecordClass.getField('relations')) {
            var operators = [];
            Ext.each(Klb.system.widgets.relation.Manager.get(this.app, this.ownRecordClass, this.ignoreRelatedModels), function(relation) {
                if (Klb.system.Common.hasRight('run', relation.relatedApp)) {
                    // TODO: leave label as it is?
                    var label = relation.text.replace(/ \(.+\)/,'');
                    operators.push({operator: {linkType: 'relation', foreignRecordClass: Klb.system.Common.resolveModel(relation.relatedModel, relation.relatedApp)}, label: label});
                }
            }, this);
        }
        // get operators from registry
        Ext.each(Klb.system.widgets.grid.ForeignRecordFilter.OperatorRegistry.get(this.ownRecordClass), function(def) {
            // translate label
            var foreignRecordClass = Klb.system.data.RecordMgr.get(def.foreignRecordClass),
        ...
        this.operatorStore = new Klb.system.data.JsonStore({
    ...
    initExplicit: function() {
        ...
        var foreignApp = Klb.system.appMgr.get(this.foreignRecordClass.getMeta('appName')),
            i18n;
        if (foreignApp) {
            i18n = foreignApp.i18n;
        } else {
            i18n = new Klb.system.Locale.Gettext();
            i18n.textdomain('Modern');
        }
    ...
    setRelatedRecordValue: function(filter) {
        ...
        if (this.isGeneric) {
            ...
            if (! value.length)  {
                ...
                Klb.Logger.info('hide row -> not yet implemented');
            }
    ...
    createRelatedRecordToolbar: function(filter) {
        ...
        if (! filter.toolbar) {
            ...
            filter.toolbar = new Klb.system.widgets.grid.FilterToolbar({
    ...
    operatorRenderer: function (filter, el) {
        ...
        if (! this.isGeneric) {
            operator = Klb.system.widgets.grid.ForeignRecordFilter.superclass.operatorRenderer.apply(this, arguments);
            ...
            operator = new Ext.form.field.ComboBox({
                filter: filter,
                width: 80,
        ...
        operator.origGetValue = operator.getValue.bind(operator);
...
//        var origSetValue = operator.setValue.bind(operator);
    ...
    valueRenderer: function(filter, el) {
        ...
        switch(operator) {
            case 'equals':
                ...
                value = Klb.system.widgets.form.RecordPickerManager.get(this.foreignRecordClass.getMeta('appName'), this.foreignRecordClass, Ext.apply({
                    filter: filter,
                    blurOnSelect: true,
                ...
                value.origSetValue = value.setValue.bind(value);
                break;
            ...
            default: 
                ...
                if (! filter.formFields.value) {
                    ...
                    // show button
                    el.addCls('x-btn-over');
                    
//                    value.getValue = this.getRelatedRecordValue.bind(this, [filter]);
//                    value.setValue = this.setRelatedRecordValue.bind(this, [filter]);
        ...
        value.setValue = this.setRelatedRecordValue.bind(this, [filter], 0);
        value.getValue = this.getRelatedRecordValue.bind(this, [filter]);
...
//            this.subFilterModels.push(Klb.system.widgets.grid.FilterToolbar.prototype.createFilterModel.call(this, config));
//        }, this);
//        
...
/**
 * @namespace   Klb.system.widgets.grid
 * @class       Klb.system.widgets.grid.FilterRegistry
 * @singleton
 */
Klb.system.widgets.grid.ForeignRecordFilter.OperatorRegistry = function() {
    var operators = {};
...
Klb.system.statics.Filters.add('foreignrecord', 'Klb.system.widgets.grid.ForeignRecordFilter');
