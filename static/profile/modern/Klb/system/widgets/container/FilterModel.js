 ...
 * 
 * TODO         extend Klb.system.widgets.grid.PickerFilter
 */
Ext.ns('Klb.system.widgets.container');

/**
 * @namespace   Klb.system.widgets.container
 * @class       Klb.system.widgets.container.FilterModel
 * @extends     Klb.system.widgets.grid.FilterModel
 * 
 * @author      Cornelius Weiss <c.weiss@metaways.de>
 */
Ext.define('Klb.system.widgets.container.FilterModel', {
     ...
     * 
     * @cfg {Klb.system.Application} callingApp
     */
    callingApp: null,
    ...
    initComponent: function() {
        ...
        if (Ext.isString(this.recordClass)) {
            ...
            this.recordClass = Klb.App[split[0]].Model[split[1]];
        }
        
        // get application if string is given
        if (Ext.isString(this.app)) {
            this.app = Klb.system.appMgr.get(this.app);
        }
        
        // get calling application if string is given
        if (Ext.isString(this.callingApp)) {
            this.callingApp = Klb.system.appMgr.get(this.callingApp);
        }
    ...
    valueRenderer: function(filter, el) {
        ...
        filter.valueFields = {};
        filter.valueFields.selectionComboBox = new Klb.system.widgets.container.selectionComboBox({
            ...
            setValue: function(value) {
                if (this.filter.get('operator') !== 'in') {
                    ...
                    if (value.path && value.path === Klb.system.Container.getMyNodePath()) {
                        operatorText = _('is equal to')
                    }
                    this.filter.formFields.operator.setRawValue(operatorText);
                }
            
                return Klb.system.widgets.container.selectionComboBox.prototype.setValue.call(this, value);
            },
            listeners: {
                scope: this, 
        ...
        filter.valueFields.FilterModelMultipleValueField = new Klb.system.widgets.container.FilterModelMultipleValueField({
...
/**
 * @namespace   Klb.system.widgets.container
 * @class       Klb.system.widgets.container.FilterModelMultipleValueField
 * @extends     Ext.ux.form.LayerCombo
 * 
 * @author      Cornelius Weiss <c.weiss@metaways.de>
 */
Ext.define('Klb.system.widgets.container.FilterModelMultipleValueField', {
...
//        this.supr().initComponent.call(this);
    ...
    getColumnModel: function() {
        return new Ext.grid.ColumnManager({
            ...
            columns:  [
                {id: 'name', header:  Ext.String.format(_('Selected  {0}'), this.containersName), dataIndex: 'name'}
            ]
        });
    },
    ...
    getItems: function() {
        var containerSelectionCombo = this.containerSelectionCombo = new Klb.system.widgets.container.selectionComboBox({
            allowNodeSelect: true,
            allowBlank: true,
            blurOnSelect: true,
            recordClass: this.recordClass,
        ...
        this.containerGridPanel = new Klb.system.widgets.grid.PickerGridPanel({
            height: this.layerHeight || 'auto',
            recordClass: Klb.system.Model.Container,
            store: this.store,
    ...
    onContainerSelect: function(field, containerData) {
        ...
        if (! existingRecord) {
            ...
            this.store.add(new Klb.system.Model.Container(data));
    ...
    setValue: function(value) {
        ...
        Ext.each(value, function(containerData) {
            ...
            if (containerData.path && containerData.path === Klb.system.Container.getMyNodePath()) {
                containerData.name = null;
            }
            containerData.name = containerData.name || Klb.system.Container.path2name(containerData.path, this.containerName, this.containersName);
            containerData.id = containerData.id ||containerData.path;
            
            this.store.add(new Klb.system.Model.Container(containerData));
...
Klb.system.statics.Filters.add('apibase.widget.container.filtermodel', 'Klb.system.widgets.container.FilterModel' );