 ...
 * @class       Klb.system.widgets.notes.PickerCombo
 * @author      Alexander Stintzing <a.stintzing@metaways.de>
 */

Ext.define('Klb.system.widgets.notes.PickerCombo', {
    extend: 'Ext.Container',

    xtype: 'apibaserelationpickercombo',
    // private
    items: null,
    combo: null,
    layout: 'fit',
    ...
    deferredLoading: function() {
        ...
        this.combo.on('select', function() {
            ...
            if(value.length) {
                ...
                if(this.store.findBy(function(record, id) {
                    if(record) {
                        if(record.get('noted_id') == value && record.get('type') == this.relationType) return true;
                    }
                }, this) == -1) {
                    var relationRecord = new Klb.system.Model.Relation(Ext.apply(this.editDialog.relationsPanel.getRelationDefaults(), {
                        noted_record: recordToAdd.data,
                        noted_id: recordToAdd.id,
                        noted_model: this.fullModelName,
        ...
        Klb.system.widgets.notes.PickerCombo.superclass.initComponent.call(this);
    },

    /**
     * removes the relation record by the noted record id, if the type is the same handled by this combo
     * @param {String} id
     */
    removeIdFromStore: function(id) {
        if(id) {
            var oldRecPos = this.store.findBy(function(record) {
                if(record) {
                    if(record.get('noted_id') == id && record.get('type') == this.relationType) return true;
                } else {
                    return false;
                }
            }, this);
