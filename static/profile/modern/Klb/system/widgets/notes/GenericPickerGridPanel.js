 ...
 *
 */
Ext.ns('Klb.system.widgets.notes');
/**
...
Ext.define('Klb.system.widgets.notes.GenericPickerGridPanel', {
    ...
    initComponent: function() {
        ...
        if (this.relationPanelRegistry && this.relationPanelRegistry.length) {
            Ext.each(this.relationPanelRegistry, function(panel) {
                if (panel.notedPhpModel) {
                    this.ignoreRelatedModels.push(panel.notedPhpModel);
                }
            }, this);
        }
        
        this.possibleRelations = Klb.system.widgets.notes.Manager.get(this.app, this.ownRecordClass, this.ignoreRelatedModels);
            ...
            if (rel.ownModel == this.ownRecordClass.getMeta('modelName')) {
                if (rel.keyfieldConfig) {
                    if (rel.keyfieldConfig.from == 'foreign') {
                        this.keyFieldConfigs[rel.notedApp + rel.notedModel] = {app: rel.notedApp, name: rel.keyfieldConfig.name};
                    } else {
                        this.keyFieldConfigs[this.app.name + rel.ownModel] = {app: this.app.name, name: rel.keyfieldConfig.name};
                    }
                }
                if (rel.config) {
                    this.constrainsConfig[rel.notedApp + rel.notedModel] = rel.config;
                }
            }
        }, this);
        ...
        Klb.system.widgets.notes.GenericPickerGridPanel.superclass.initComponent.call(this);
    ...
    onSaveRecord: function(dialog, record, ticket) {
        ...
        if (this.isValid()) {
            ...
            Ext.each(this.relationPanelRegistry, function(panel) {
                notes = notes.concat(this.getData(panel.store));
            }, this);
            
            notes = notes.concat(this.getData());
            
            record.set('notes', notes);
        } else {
            Ext.Msg.alert(_('Relations failure'), _('There are invalid notes. Please check before saving.'));
    ...
    getViewRowClass: function(record, index, rowParams, store) {
        if(this.invalidRowRecords && this.invalidRowRecords.indexOf(record.id) !== -1) {
            var model = record.get('noted_model').split('_Model_');
    ...
    onEditInNewWindow: function() {
        var selected = this.getSelectionModel().getSelected(),
            app = selected.get('noted_model').split('_')[0],
            model = selected.get('noted_model').split('_')[2],
            ms = Klb.system.appMgr.get(app).getMainScreen(),
            recordData = selected.get('noted_record'),
     ...
     * adds additional menu items from Klb.system.widgets.notes.MenuItemManager
    ...
    onBeforeContextMenu: function(grid, index) {
        ...
        var additionalItems = Klb.system.widgets.notes.MenuItemManager.get(model[0], model[1], {
            scope: app,
            grid: grid,
            gridIndex: index
        });
    ...
    getModelCombo: function() {
        if (!this.modelCombo) {
            ...
            Ext.each(this.possibleRelations, function(rel) {
                data.push([id, rel.text, rel.notedApp, rel.notedModel]);
                id++;
            }, this);
    ...
    createSearchCombos: function() {
        ...
        Ext.each(this.possibleRelations, function(rel) {
            var key = rel.notedApp+rel.notedModel;
            this.searchCombos[key] = Klb.system.widgets.form.RecordPickerManager.get(rel.notedApp, rel.notedModel,{
                width: 300,
                allowBlank: true,
                listeners: {
                    scope: this,
        ...
        this.showSearchCombo(this.possibleRelations[0].notedApp, this.possibleRelations[0].notedModel);
        return sc;
    },
    /**
    ...
    getColumnModel: function () {
        ...
        if (! this.colModel) {
            this.colModel = new Ext.grid.ColumnManager({
                ...
                columns: [
                    {id: 'noted_model', dataIndex: 'noted_model', header: _('Record'), editor: false, renderer: this.notedModelRenderer.bind(this), scope: this},
                    {id: 'noted_record', dataIndex: 'noted_record', header: _('Description'), renderer: this.notedRecordRenderer.bind(this), editor: false, scope: this},
                    {id: 'note', dataIndex: 'note', header: _('Remark'), renderer: this.noteRenderer.bind(this), editor: Ext.form.Field, scope: this, width: 120},
    ...
    onBeforeRowEdit: function(o) {
        var model = o.record.get('noted_model').split('_');
        var app = model[0];
        model = model[2];
    ...
    /**
     * noted record renderer
     *
     * @param {Record} value
     * @return {String}
     */
    notedRecordRenderer: function (recData, meta, relRec) {
        var relm = relRec.get('noted_model');
        if(!relm) {
            return '';
        }
     ...
     * @return {String}
     */
    noteRenderer: function(value, row, record) {
        if (record && record.get('noted_model')) {
            var app = Klb.system.appMgr.get(record.get('noted_model').split('_Model_')[0]);
        }
        if (! value) {
            value = '';
            ...
            return this.noteRenderer(value);
        }
     ...
     * @return {String}
     */
    notedModelRenderer: function(value) {
        if(!value) {
            return '';
        }
    ...
    typeRenderer: function(value, row, rec) {
        if(!rec.get('own_model') || !rec.get('noted_model')) {
            return '';
        }
        var o = rec.get('own_model').split('_Model_').join('');
        var f = rec.get('noted_model').split('_Model_').join('');
    ...
    onAddRecord: function(record, relconf) {
        if (record) {
            ...
            var notedPhpModel = rc.getPhpClassName();
            ...
            var relationRecord = new Klb.system.Model.Relation(Ext.apply(this.getRelationDefaults(), Ext.apply({
                noted_record: record.data,
                noted_id: record.id,
                noted_model: notedPhpModel,
                type: type,
                own_degree: 'sibling'
            }, relconf)), record.id);
                ...
                Ext.each(this.constrainsConfig[app + model], function(conf) {
                    ...
                    if (conf.hasOwnProperty('max') && conf.max > 0 && (conf.type == relationRecord.get('type'))) {
                        var resNew = this.store.queryBy(function(existingRecord, id) {
                            if ((relationRecord.get('type') == existingRecord.get('type')) && (existingRecord.get('noted_model') == relationRecord.get('noted_model'))) {
                                return true;
                            } else {
                                return false;
                            }
            ...
            if (this.store.findExact('noted_id', record.id) === -1) {
     ...
     * @param {String} notedModel
     * @return {Boolean}
     */
    relationCheck: function(recordToAdd, notedModel) {
        var add = true;
        this.store.each(function(relation) {
            if (relation.get('noted_model') == notedModel && relation.get('noted_id') == recordToAdd.getId()) {
                Ext.MessageBox.show({
                    title: _('Failure'),
        ...
        if((this.ownRecordClass.getMeta('phpClassName') == notedModel) && recordToAdd.getId() == this.editDialog.record.getId()) {
            Ext.MessageBox.show({
                title: _('Failure'),
                ...
                Ext.each(this.constrainsConfig[app + model], function(conf) {
                    ...
                    if(conf.max && conf.max > 0 && (conf.type == o.value)) {
                        var resNew = this.store.queryBy(function(record, id) {
                            if((o.value == record.get('type')) && (record.get('noted_model') == (app + '_Model_' + model))) return true;
                            else return false;
                        }, this);
                    ...
                    if (conf.max && conf.max > 0 && (conf.type == o.originalValue)) {
                        var resOld = this.store.queryBy(function(record, id) {
                            if((o.originalValue == record.get('type')) && (record.get('noted_model') == (app + '_Model_' + model))) return true;
                            else return false;
                        }, this);
    ...
    onAdd: function(store, records) {
        Ext.each(records, function(record) {
            Ext.each(this.editDialog.relationPickers, function(picker) {
                if(picker.relationType == record.get('type') && record.get('noted_id') != picker.getValue() && picker.fullModelName == record.get('noted_model')) {
                    var split = picker.fullModelName.split('_Model_');
                    picker.combo.selectedRecord = new Klb.App[split[0]].Model[split[1]](record.get('noted_record'));
                    picker.combo.startRecord = new Klb.App[split[0]].Model[split[1]](record.get('noted_record'));
    ...
    onUpdate: function(store, record) {
        store.each(function(record) {
            Ext.each(this.editDialog.relationPickers, function(picker) {
                if(picker.relationType == record.get('type') && picker.fullModelName == record.get('noted_model')) {
                    picker.setValue(record.get('noted_record'));
                }
            }, this);
        }, this);
        this.updateTitle();
    ...
    loadRecord: function(dialog, record, ticketFn) {
        ...
        if (notes && notes.length > 0) {
            ...
            Ext.each(notes, function(relation) {
                if(this.ignoreRelatedModels) {
                    if(relation.hasOwnProperty('noted_model') && this.ignoreRelatedModels.indexOf(relation.noted_model) == -1) {
    ...
    /**
     * checks if there are invalid notes
     * @return {Boolean}
     */
    isValid: function() {
    ...
    getData: function(store) {
        ...
        var notes = [];
        store.each(function(record) {
            delete record.data.noted_record.notes;
            delete record.data.noted_record.relation;
