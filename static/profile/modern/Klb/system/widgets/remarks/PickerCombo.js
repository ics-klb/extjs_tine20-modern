 ...
 * @class       Klb.system.widgets.remarks.PickerCombo
 * @author      Alexander Stintzing <a.stintzing@metaways.de>
 */

Klb.system.widgets.remarks.PickerCombo = Ext.extend(Ext.Container, {
    ...
    deferredLoading: function() {
        ...
        this.combo.on('select', function() {
            ...
            if(value.length) {
                ...
                if(this.store.findBy(function(record, id) {
                    if(record) {
                        if(record.get('remarkd_id') == value && record.get('type') == this.relationType) return true;
                    }
                }, this) == -1) {
                    var relationRecord = new Klb.system.Model.Relation(Ext.apply(this.editDialog.relationsPanel.getRelationDefaults(), {
                        remarkd_record: recordToAdd.data,
                        remarkd_id: recordToAdd.id,
                        remarkd_model: this.fullModelName,
        ...
        Klb.system.widgets.remarks.PickerCombo.superclass.initComponent.call(this);
    },

    /**
     * removes the relation record by the remarkd record id, if the type is the same handled by this combo
     * @param {String} id
     */
    removeIdFromStore: function(id) {
        if(id) {
            var oldRecPos = this.store.findBy(function(record) {
                if(record) {
                    if(record.get('remarkd_id') == id && record.get('type') == this.relationType) return true;
                } else {
                    return false;
                }
            }, this);
...
//Ext.reg('tinerelationpickercombo', Klb.system.widgets.remarks.PickerCombo);