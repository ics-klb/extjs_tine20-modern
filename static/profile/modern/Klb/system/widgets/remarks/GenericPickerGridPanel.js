 ...
 * @author      Alexander Stintzing <a.stintzing@metaways.de>
 */
Ext.define('Klb.system.widgets.remarks.GenericPickerGridPanel', {
    ...
    initComponent: function() {
        ...
        if (this.relationPanelRegistry && this.relationPanelRegistry.length) {
            Ext.each(this.relationPanelRegistry, function(panel) {
                if (panel.remarkdPhpModel) {
                    this.ignoreRelatedModels.push(panel.remarkdPhpModel);
                }
            }, this);
        }
        
        this.possibleRelations = Klb.system.widgets.remarks.Manager.get(this.app, this.ownRecordClass, this.ignoreRelatedModels);
            ...
            if (rel.ownModel == this.ownRecordClass.getMeta('modelName')) {
                if (rel.keyfieldConfig) {
                    if (rel.keyfieldConfig.from == 'foreign') {
                        this.keyFieldConfigs[rel.remarkdApp + rel.remarkdModel] = {app: rel.remarkdApp, name: rel.keyfieldConfig.name};
                    } else {
                        this.keyFieldConfigs[this.app.name + rel.ownModel] = {app: this.app.name, name: rel.keyfieldConfig.name};
                    }
                }
                if (rel.config) {
                    this.constrainsConfig[rel.remarkdApp + rel.remarkdModel] = rel.config;
                }
            }
        }, this);
        ...
        Klb.system.widgets.remarks.GenericPickerGridPanel.superclass.initComponent.call(this);
    ...
    onSaveRecord: function(dialog, record, ticket) {
        ...
        if (this.isValid()) {
            if (record.data.hasOwnProperty('remarks')) {
                delete record.data.remarks;
            }
            var remarks = [];
            
            Ext.each(this.relationPanelRegistry, function(panel) {
                remarks = remarks.concat(this.getData(panel.store));
            }, this);
            
            remarks = remarks.concat(this.getData());
            
            record.set('remarks', remarks);
        } else {
            Ext.Msg.alert(_('Relations failure'), _('There are invalid remarks. Please check before saving.'));
    ...
    getViewRowClass: function(record, index, rowParams, store) {
        if(this.invalidRowRecords && this.invalidRowRecords.indexOf(record.id) !== -1) {
            var model = record.get('remarkd_model').split('_Model_');
    ...
    onEditInNewWindow: function() {
        var selected = this.getSelectionModel().getSelected(),
            app = selected.get('remarkd_model').split('_')[0],
            model = selected.get('remarkd_model').split('_')[2],
            ms = Klb.system.appMgr.get(app).getMainScreen(),
            recordData = selected.get('remarkd_record'),
     ...
     * adds additional menu items from Klb.system.widgets.remarks.MenuItemManager
    ...
    onBeforeContextMenu: function(grid, index) {
        ...
        var additionalItems = Klb.system.widgets.remarks.MenuItemManager.get(model[0], model[1], {
            scope: app,
            grid: grid,
            gridIndex: index
        });
    ...
    getModelCombo: function() {
        if (!this.modelCombo) {
            ...
            Ext.each(this.possibleRelations, function(rel) {
                data.push([id, rel.text, rel.remarkdApp, rel.remarkdModel]);
                id++;
            }, this);
    ...
    createSearchCombos: function() {
        ...
        Ext.each(this.possibleRelations, function(rel) {
            var key = rel.remarkdApp+rel.remarkdModel;
            this.searchCombos[key] = Klb.system.widgets.form.RecordPickerManager.get(rel.remarkdApp, rel.remarkdModel,{
                width: 300,
                allowBlank: true,
                listeners: {
                    scope: this,
        ...
        this.showSearchCombo(this.possibleRelations[0].remarkdApp, this.possibleRelations[0].remarkdModel);
        return sc;
    },
    /**
    ...
    getColumnModel: function () {
        ...
        if (! this.colModel) {
            this.colModel = new Ext.grid.ColumnManager({
                defaults: {
                    sortable: true,
                    width: 180
                },
                columns: [
                    {id: 'remarkd_model', dataIndex: 'remarkd_model', header: _('Record'), editor: false, renderer: this.remarkdModelRenderer.bind(this), scope: this},
                    {id: 'remarkd_record', dataIndex: 'remarkd_record', header: _('Description'), renderer: this.remarkdRecordRenderer.bind(this), editor: false, scope: this},
                    {id: 'remark', dataIndex: 'remark', header: _('Remark'), renderer: this.remarkRenderer.bind(this), editor: Ext.form.Field, scope: this, width: 120},
                    {id: 'own_degree', hidden: true, dataIndex: 'own_degree', header: _('Dependency'), editor: this.degreeEditor, renderer: this.degreeRenderer.bind(this), scope: this, width: 100},
                    {id: 'type', dataIndex: 'type', renderer: this.typeRenderer, header: _('Type'),  scope: this, width: 120, editor: true},
                    {id: 'creation_time', dataIndex: 'creation_time', editor: false, renderer: Klb.system.Common.dateTimeRenderer, header: _('Creation Time'), width: 140}
                ]
            });
        }
    ...
    onBeforeRowEdit: function(o) {
        var model = o.record.get('remarkd_model').split('_');
        var app = model[0];
        model = model[2];
    ...
    /**
     * remarkd record renderer
     *
     * @param {Record} value
     * @return {String}
     */
    remarkdRecordRenderer: function (recData, meta, relRec) {
        var relm = relRec.get('remarkd_model');
        if(!relm) {
            return '';
        }
    ...
    remarkRenderer: function(value, row, record) {
        if (record && record.get('remarkd_model')) {
            var app = Klb.system.appMgr.get(record.get('remarkd_model').split('_Model_')[0]);
        }
        if (! value) {
            value = '';
     ...
     * @return {String}
     */
    remarkdModelRenderer: function(value) {
        if(!value) {
            return '';
        }
    ...
    typeRenderer: function(value, row, rec) {
        if(!rec.get('own_model') || !rec.get('remarkd_model')) {
            return '';
        }
        var o = rec.get('own_model').split('_Model_').join('');
        var f = rec.get('remarkd_model').split('_Model_').join('');
    ...
    onAddRecord: function(record, relconf) {
        if (record) {
            ...
            var remarkdPhpModel = rc.getPhpClassName();
            ...
            var relationRecord = new Klb.system.Model.Relation(Ext.apply(this.getRelationDefaults(), Ext.apply({
                remarkd_record: record.data,
                remarkd_id: record.id,
                remarkd_model: remarkdPhpModel,
                type: type,
                own_degree: 'sibling'
            }, relconf)), record.id);
                ...
                Ext.each(this.constrainsConfig[app + model], function(conf) {
                    ...
                    if (conf.hasOwnProperty('max') && conf.max > 0 && (conf.type == relationRecord.get('type'))) {
                        var resNew = this.store.queryBy(function(existingRecord, id) {
                            if ((relationRecord.get('type') == existingRecord.get('type')) && (existingRecord.get('remarkd_model') == relationRecord.get('remarkd_model'))) {
                                return true;
                            } else {
                                return false;
                            }
            ...
            if (this.store.findExact('remarkd_id', record.id) === -1) {
     ...
     * @param {String} remarkdModel
     * @return {Boolean}
     */
    relationCheck: function(recordToAdd, remarkdModel) {
        var add = true;
        this.store.each(function(relation) {
            if (relation.get('remarkd_model') == remarkdModel && relation.get('remarkd_id') == recordToAdd.getId()) {
                Ext.MessageBox.show({
                    title: _('Failure'),
        ...
        if((this.ownRecordClass.getMeta('phpClassName') == remarkdModel) && recordToAdd.getId() == this.editDialog.record.getId()) {
            Ext.MessageBox.show({
                title: _('Failure'),
                ...
                Ext.each(this.constrainsConfig[app + model], function(conf) {
                    ...
                    if(conf.max && conf.max > 0 && (conf.type == o.value)) {
                        var resNew = this.store.queryBy(function(record, id) {
                            if((o.value == record.get('type')) && (record.get('remarkd_model') == (app + '_Model_' + model))) return true;
                            else return false;
                        }, this);
                    ...
                    if (conf.max && conf.max > 0 && (conf.type == o.originalValue)) {
                        var resOld = this.store.queryBy(function(record, id) {
                            if((o.originalValue == record.get('type')) && (record.get('remarkd_model') == (app + '_Model_' + model))) return true;
                            else return false;
                        }, this);
    ...
    onAdd: function(store, records) {
        Ext.each(records, function(record) {
            Ext.each(this.editDialog.relationPickers, function(picker) {
                if(picker.relationType == record.get('type') && record.get('remarkd_id') != picker.getValue() && picker.fullModelName == record.get('remarkd_model')) {
                    var split = picker.fullModelName.split('_Model_');
                    picker.combo.selectedRecord = new Klb.App[split[0]].Model[split[1]](record.get('remarkd_record'));
                    picker.combo.startRecord = new Klb.App[split[0]].Model[split[1]](record.get('remarkd_record'));
    ...
    onUpdate: function(store, record) {
        store.each(function(record) {
            Ext.each(this.editDialog.relationPickers, function(picker) {
                if(picker.relationType == record.get('type') && picker.fullModelName == record.get('remarkd_model')) {
                    picker.setValue(record.get('remarkd_record'));
                }
            }, this);
        }, this);
        this.updateTitle();
    ...
    loadRecord: function(dialog, record, ticketFn) {
        ...
        if (remarks && remarks.length > 0) {
            this.updateTitle(remarks.length);
            var relationRecords = [];
            
            Ext.each(remarks, function(relation) {
                if(this.ignoreRelatedModels) {
                    if(relation.hasOwnProperty('remarkd_model') && this.ignoreRelatedModels.indexOf(relation.remarkd_model) == -1) {
    ...
    /**
     * checks if there are invalid remarks
     * @return {Boolean}
     */
    isValid: function() {
    ...
    getData: function(store) {
        ...
        var remarks = [];
        store.each(function(record) {
            delete record.data.remarkd_record.remarks;
            delete record.data.remarkd_record.relation;
            remarks.push(record.data);
        }, this);
