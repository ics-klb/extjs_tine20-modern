    ...
    public function authenticate($_expired_check = NULL)
    {
        ...
        foreach ($this->_options as $name => $options) {
            ...
            try {
                ...
                if($_expired_check)
                {
                    Modern_Api_Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . " LDAP backend is setted up to check password expiration");
                    $user_pass_expired = $ldap->passwordExpired($username);
                    
                    if(is_array($user_pass_expired))
                    {
                        //$err = new Zend_Ldap_Exception($ldap, $user_pass_expired[1], Zend_Ldap_Exception::LDAP_PASSWORD_EXPIRED);
                        $err = new Zend_Ldap_Exception($ldap, $user_pass_expired[1], Zend_Ldap_Exception::LDAP_PASSWORD_EXPIRE_ATTRIBUTE_WRONG);
                        throw($err);
                    }
                
                    if($user_pass_expired)
                    {
                        if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP password for user '$username' is expired");
                        $options = $this->getLdap()->getOptions();
                        $options['passwordExpired'] = true;
                        $err = new Zend_Ldap_Exception($ldap, "$canonicalName authentication fails: user password has expired", Zend_Ldap_Exception::LDAP_PASSWORD_EXPIRED);
                        throw($err);
                    }
                    else
                    {
                        if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP password for user '$username' is valid, and does not prescribed date");
                        $messages[0] = '';
                        $messages[1] = '';
                        ...
                        Modern_Api_Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . " LDAP backend is not configured to check password expiration");
                        $messages[0] = '';
                        $messages[1] = '';
                        $messages[] = "$canonicalName authentication successful";
                        return new Zend_Auth_Result(Zend_Auth_Result::SUCCESS, $canonicalName, $messages);                    
                ...
                if ($err == Zend_Ldap_Exception::LDAP_X_DOMAIN_MISMATCH) {
                    ...
                    $failedAuthorities[$dname] = $zle->getMessage();
                } else if ($err == Zend_Ldap_Exception::LDAP_PASSWORD_EXPIRED) {
                    $code = Zend_Auth_Result::FAILURE_PASSWORD_EXPIRED;
                    $messages[0] = 'Your password has expired. You must change it. Do you want to do this now?';
                    $failedAuthorities[$dname] = $zle->getMessage();
                } else if ($err == Zend_Ldap_Exception::LDAP_PASSWORD_EXPIRE_ATTRIBUTE_WRONG) {
                    $code = Zend_Auth_Result::FAILURE_EXPIRATION_ATTRIBUTE_WRONG;
