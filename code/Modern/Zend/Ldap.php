...
class Zend_Ldap
{
    ...
    /**
     * Returns a date/time string value from LDAP default date/time format
     *
     * @param  string $_strLdapDTData
     * @param  int $_expirationTime
     * @return string
     */    
    protected function _LdapDTFormatToDate($_strLdapDTData, $_expirationTime){
        $dateformat = substr($_strLdapDTData, 0,4) . "-" . substr($_strLdapDTData, 4, 2) . "-" . substr($_strLdapDTData, 6, 2) . " " . substr($_strLdapDTData, 8, 2) . ":" . substr($_strLdapDTData, 10, 2) . ":" . substr($_strLdapDTData, 12, 2);
        $d_ldap = new DateTime($dateformat);
        $d_ldap->add(new DateInterval('P'.$_expirationTime.'D'));
        return($d_ldap);
    }    
     ...
     * @throws Zend_Ldap_Exception
     */      
    protected function _isPasswordExpired($_acctname){        
        ...
        $attrs = array(Modern_Api_User::getBackendConfiguration("passwordExpirationAttribute"));
        
        if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP will search for password expiration attribute '".Modern_Api_User::getBackendConfiguration("passwordExpirationAttribute")."' for user '".$_acctname."' authentication");
        
        $fetch_data = $this->search($accountFilter, $baseDn, self::SEARCH_SCOPE_SUB, $attrs);
        $count = $fetch_data->count();
        if ($count === 1) {
            if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP found user entry for password expiration");
            $expirationInterval = Modern_Api_User::getBackendConfiguration("passwordExpirationInterval");
            $user_data = $fetch_data->getFirst();
            $att_name = strtolower(Modern_Api_User::getBackendConfiguration("passwordExpirationAttribute"));
            
            if(!$user_data[$att_name][0])
            {
                /**
                * @see Zend_Ldap_Exception
                */
                Modern_Api_Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . " LDAP backend is configured to check password expiration, but it was not possible to find out the desired attribute('".$att_name."') name for user account ('".$_acctname."')");
                require_once 'Zend/Ldap/Exception.php';
                throw new Zend_Ldap_Exception(null, 'LDAP user account "'.$_acctname.'" does not have a password expiration attribute called "'.$att_name.'". Check setup entry for authentication section and LDAP data from fetched user', Zend_Ldap_Exception::LDAP_PARAM_ERROR);
            }

            $user_att_tmz = $user_data[$att_name][0];                
            $date_user_password_expires = $this->_LdapDTFormatToDate($user_att_tmz, $expirationInterval);
            $fetch_data->close();
            $today = date('Y-m-d H:i:s');            
            $date_today = new DateTime($today);
            if($date_user_password_expires < $date_today){
                if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP fingers user '".$_acctname."' password is expired");
                return(true);
            }
            else{
                if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP fingers user '".$_acctname."' has not expired password");
                return(false);
            }
        } else if ($count === 0) {
            /**
             * @see Zend_Ldap_Exception
             */
            Modern_Api_Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . " LDAP did not found user entry attibute match for password expiration to user '".$this->_options['username']."'");
            require_once 'Zend/Ldap/Exception.php';
            $code = Zend_Ldap_Exception::LDAP_NO_SUCH_OBJECT;
            $str = "No object found for: $accountFilter";
        } else {
             ...
             * @see Zend_Ldap_Exception
             */
            Modern_Api_Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . " LDAP fails trying to check password expiration attribute for user authentication with credencials '".$this->_options['username']."'");
            require_once 'Zend/Ldap/Exception.php';
            $code = Zend_Ldap_Exception::LDAP_OPERATIONS_ERROR;
            $str = "Unexpected result count ($count) for: $accountFilter";
        }
     ...
     /**
     * Returns user password expired status
     *
     * @param  string $uid 
     * @return boolean (success) array(failure)
     * @throws Zend_Ldap_Exception
     */
    public function passwordExpired($uid){
        if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG)) Modern_Api_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " LDAP check for password expiration for user '$uid'");
        try{
            $is_expired = $this->_isPasswordExpired($uid);
            return($is_expired);
        }
