 ...
 */
require_once APP_EXTENDCODE_PATH . '/Zend/Server/Abstract.php';
require_once APP_EXTENDCODE_PATH . '/Zend/Server/Reflection.php';
/**
 * @category   Zend
 * @package    Zend_Json
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
...
if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG))
           Modern_Api_Core::getLogger()
              ->debug(__METHOD__ . '::' . __LINE__ .' ------------------ Request_handle ----------------: ' . $method  );

        if (!$this->_table->hasMethod($method)) {

if (Modern_Api_Core::isLogLevel(Zend_Log::DEBUG))
           Modern_Api_Core::getLogger()
              ->debug(__METHOD__ . '::' . __LINE__ .' ------------------ _handle ----------------: ' . $method );
        ...
        $_arguments = array();
        foreach($serviceParams as $value) {
            $field = $value['name'];
            ...
            if ( isset($value['type']) ) {
                ...
                switch($value['type']) {
                    ...
                    case 'int':
                    case 'integer':
                           $_arguments[$field] = intval($params[$field]);
                        break;
                    case 'boolean':
                    case 'bool':
                           $_arguments[$field] = (boolean) $params[$field];
                        break;
                    case 'az09':
                        if ( preg_match('/^[A-Za-z0-9]+(=)?$/', $params[$field] ) ) {
                            $_arguments[$field] = $params[$field];
                        } else {
                            throw new Modern_Api_Exception_InvalidArgument('Can A-Z model name only');
                        }
        ...
        try {
            $result = $this->_dispatch($invocable, $_arguments);
        } catch (Exception $e) {
