 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Modern_Scheduler_Task_Rule
{
    ...
    public function __construct($type, $rule)
    {
        ...
        if (empty($type)) {
            throw new Zend_Modern_Scheduler_Exception('Type string expected');
        }
        if (!isset($this->_times[$type])) {
            throw new Zend_Modern_Scheduler_Exception("Type '{$type}' was not a valid type");
        }
        if (empty($type)) {
            throw new Zend_Modern_Scheduler_Exception('Rule string expected');
        }
     ...
     * @return Zend_Modern_Scheduler_Task This instance
     */
    ...
    public function matches($time = '')
    {
        ...
        if (empty($this->_time)) {
            throw new Zend_Modern_Scheduler_Exception('You must supply a time');
        }
    ...
    protected function _parse()
    {
        ...
        if ($this->_type == 'months' or $this->_type == 'weekdays') {
            if (strpos($rule, '/') !== false) {
                throw new Zend_Modern_Scheduler_Exception(ucfirst($this->_type) 
        ...
        foreach ($values as $i => $value) {
            if (strpos($value, '-') !== false and strpos($value, '/') !== false) {
                throw new Zend_Modern_Scheduler_Exception('Invalid ' . $this->_type . ' value');
            }
            ...
            if (preg_match('/\A\d+\s*\-\s*\d+\Z/', $value)) {
                ...
                if (!$this->_inRange($valueFrom) or !$this->_inRange($valueTo)) {
                    throw new Zend_Modern_Scheduler_Exception('Invalid ' . $this->_type . ' range');
                }
