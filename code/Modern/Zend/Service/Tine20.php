...
class Zend_Service_Tine20 extends Zend_Json_Client
{
    ...
    /**
     * the url of the Modern 3.1.0 installation
     * 
     * @var string (for example http://demo.modern30.org/index.php)
     */
    protected $_url;
     ...
     * @param string           $url         the url of the Modern 3.1.0 installation
    ...
    /**
     * login to Modern 3.1.0 installation
     * 
    ...
    public function login($loginname, $password)
    {
        ...
        $response = $this->call('Modern.login', array(
            'username'  => $loginname,
            'password'  => $password
        ));
        ...
        if($this->_cache instanceof Zend_Cache_Core) {
            if($this->_cache->test('modern30PrivateSMD')) {
                $smd = $this->_cache->load('modern30PrivateSMD');
                $this->getIntrospector()->setSMD($smd);
            } else {
                $smd = $this->getIntrospector()->fetchSMD();
                $this->_cache->save($smd, 'modern30PrivateSMD');
            }
        } else {
    ...
    /**
     * logout from Modern 3.1.0 installation
     * 
     * @return array decoded JSON responce
     */
    public function logout()
    {
        ...
        $response = $this->call('Modern.logout');
