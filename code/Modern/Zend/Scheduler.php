...
class Zend_Scheduler
{
    ...
    /** @var Zend_Modern_Scheduler_Backend_Abstract Backend */
    protected $_backend = null;

    /**
     * Constructor.
     * 
     * @param Zend_Modern_Scheduler_Backend_Abstract|string $backend Backend or name of backend ('File', etc.)
    ...
    public function setBackend($backend, array $options = array())
    {
        if (is_string($backend)) {
            ...
            if (in_array($backendName, self::$availableBackends)) {
                $class = 'Zend_Modern_Scheduler_Backend_' . $backendName;
                $backend = new $class($options);
            } else {
                try { // to load user-implemented backend interface
                    $backend = new $backend($options);
                } catch (Zend_Exception $e) {
                    throw new Zend_Modern_Scheduler_Exception('Invalid backend');
                }
            }
        } // $backend is now an object

        if (!$backend instanceof Zend_Modern_Scheduler_Backend_Abstract) {
            throw new Zend_Modern_Scheduler_Exception('Backend must extend Zend_Modern_Scheduler_Backend_Abstract');
        }
     ...
     * Return used Backend.
     * 
     * @return Zend_Modern_Scheduler_Backend_Abstract
     */
    ...
    public function addConfig(Zend_Config $config, $section) 
    {
        if ($config->{$section} === null) {
            throw new Zend_Modern_Scheduler_Exception("No task configuration in section '{$section}'");
        }

        // Parse task information
        foreach ($config->{$section} as $taskName => $taskData) {
            $class = isset($taskData->type) ? $taskData->type : 'Zend_Modern_Scheduler_Task';
            $task  = new $class();
            ...
            foreach ($taskData->requests as $requestName => $requestData) {
                if ($requestData->controller === null) {
                    throw new Zend_Modern_Scheduler_Exception("No controller specified for request '{$taskName}:{$requestName}'");
                }
     ...
     * @param  Zend_Modern_Scheduler_Task $task
     * @return Zend_Scheduler This instance
     */
    public function addTask($name, Zend_Modern_Scheduler_Task $task)
    {
    ...
    public function run()
    {
        if ($this->_limit and $this->_backend === null) {
            throw new Zend_Modern_Scheduler_Exception('If a limit is set, a backend must be specified');
        }
    ...
    {
        throw new Zend_Modern_Scheduler_Exception('Only tasks may be serialized');
    }
    
    /**
     * Merge Tasks from Backend and programmatically
     * 
     * @return null|Array of Zend_Modern_Scheduler_Task objects
     */
